...
import argparse
description = ('Load a telemetry log and use position data to create mission '
               'waypoints for a vehicle. '
...
def distance_to_current_waypoint():
    ...
    if nextwaypoint == 0:
        return None
    missionitem = vehicle.commands[nextwaypoint - 1]  # cmds are zero indexed
    lat = missionitem.x
    lon = missionitem.y
    alt = missionitem.z
...
def position_messages_from_tlog(filename):
    ...
    # Shrink the number of points for readability and to stay within autopilot
    # memory limits.
    ...
    while True:
        ...
        if pt2num == num_points-1 or len(kept_messages) >= 99:
            ...
            break
        pt1 = LocationGlobalRelative(messages[pt1num].lat/1.0e7,
                                     messages[pt1num].lon/1.0e7, 0)
        pt2 = LocationGlobalRelative(messages[pt2num].lat/1.0e7,
                                     messages[pt2num].lon/1.0e7, 0)
...
def arm_and_takeoff(aTargetAltitude):
    ...
    while True:
        ...
        if vehicle.location.global_relative_frame.alt >= requiredAlt:
            ...
            break
        fmt = " Altitude: %f < %f"
        print(fmt % (vehicle.location.global_relative_frame.alt, requiredAlt))
        time.sleep(1)
...
for message in messages:
    ...
    # To prevent accidents we don't trust the altitude in the original flight,
    # instead we just put in a conservative cruising altitude.
    altitude = 30.0
    cmd = Command(0,
                  0,
                  0,
                  ...
                  0, 0, 0, 0, 0, 0,
                  pt.lat/1.0e7, pt.lon/1.0e7, altitude)
    cmds.add(cmd)
