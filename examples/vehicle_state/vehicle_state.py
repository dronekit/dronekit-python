...
print("   Supports commanding position and velocity targets in local NED "
      "frame: %s" % vehicle.capabilities.set_attitude_target_local_ned)
print("   Supports set position + velocity targets in global scaled integers: "
      "%s" % vehicle.capabilities.set_altitude_target_global_int)
...
# Set vehicle home_location, mode, and armed attributes (the only settable
# attributes)

print("\nSet new home location")
# Home location must be within 50km of EKF home location (or setting will fail
# silently) In this case, just set value to current location with an easily
# recognisable altitude (222)
...
# Remove observer added with `add_attribute_listener()` specifying the
# attribute and callback function
vehicle.remove_attribute_listener('attitude', attitude_callback)


# Add mode attribute callback using decorator (callbacks added this way cannot
# be removed).
...
print("\n Attempt to remove observer added with `on_attribute` decorator "
      "(should fail)")
try:
...
print('\n'.join(" Key:%s Value:%s" % (key, value) for key, value
      in vehicle.parameters.items()))
...
# Add observer for the vehicle's any/all parameters parameter (defined using
# wildcard string ``'*'``)
