...
try:
    ...
    while True:
        ...
        # moving our vehicle around
        if gpsd.valid & gps.LATLON_SET:
            altitude = 30  # in meters
            dest = LocationGlobalRelative(gpsd.fix.latitude,
                                          gpsd.fix.longitude, altitude)
            print("Going to: %s" % dest)

            # A better implementation would only send new waypoints if the
            # position had changed significantly
            vehicle.simple_goto(dest)

            # Send a new target every two seconds. For a complete
            # implementation of follow me you would want adjust this delay.
            time.sleep(2)

except socket.error:
    print('Error: gpsd service does not seem to be running, plug in USB GPS '
          'or run run-fake-gps.sh')
    sys.exit(1)
