...
class CloudClient(object):
    ...
    def __init__(self, api_key):
        ...
        self.headers = {
            'Authorization': "DroneApi apikey=\"%s\"" % self.apikey
        }
        ...
        def method(*args):
            ...
            if (len(find_action) > 1):
                if (find_action[1] in self.REST_CALLS):
                    if (len(args) > 0):
                        action_url += "/%s/%s" % (
                            str(args[0]), self.REST_CALLS[find_action[1]])
                    else:
    ...
    def _request(self, url, data):
        self.response = requests.get("%s%s" % (self.BASE_URL, url),
                                     headers=self.headers)
        if self.response.status_code == 404:
            raise CloudError(self.response.status_code, 'Unkown Endpoint',
                             self.response)
